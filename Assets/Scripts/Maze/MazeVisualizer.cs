...
public class MazeVisualizer : MonoBehaviour
{
    ...
    {
        _maze = maze;
        var floor = CreateFloor(maze.ColumnCount, maze.RowCount, _floorMaterial);
        floor.transform.position += new Vector3(maze.ColumnCount / 2f, 0, maze.RowCount / 2f);
    ...
    private GameObject CreateWalls()
    {
        ...
        for (int i = 0; i < _maze.ColumnCount; i++)
        {
            for (int j = 0; j < _maze.RowCount; j++)
            {
                ...
                var wall = Instantiate(_w4CellPrefab, new Vector3(i, 0, j), Quaternion.identity);
                wall.Init(cell);
