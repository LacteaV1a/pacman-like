...
namespace Nox7atra.Mazes
{
    public class MazeVisualizer : MonoBehaviour
    {
        ...
        [Range(0,1),SerializeField] float _wallsvalue = 0.5f;
        [SerializeField] int _seed = 23123;
        private void Awake()
        {
            RefreshMaze();
            //GenerateW4MazeMesh(new W4Maze(_MazeCellsX, _MazeCellsY), _Height, _WallThikness, _Material);
        }
        [ContextMenu("RefreshMaze")]
        public void RefreshMaze()
        {
            //Preprocess
            var generator = new RandomEllerGenerator(_seed, _wallsvalue);
            _Maze = generator.Generate(_MazeCellsX, _MazeCellsY);
            _GraphMaze = new MazeGraph(_Maze, false);
            //Process
